...
@section('content')
    <div class="font-[Poppins]">
        ...
        <section id="search-result" class="max-w-[1130px] mx-auto flex items-start flex-col gap-[30px] mt-[70px] mb-[100px]">
            ...
            <div id="search-cards" class="grid grid-cols-3 gap-[30px] w-full">
                        ...
                        <div class="flex flex-col gap-4 p-[26px_20px] transition-all duration-300 ring-1 ring-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18] rounded-[20px] overflow-hidden bg-white">
                            <div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
                                <div class="badge absolute left-5 top-5 bottom-auto right-auto flex p-[8px_18px] bg-white rounded-[50px]">
                                    ...
                                    @elseif($article->categories->isNotEmpty())
                                        <p class="text-xs leading-[18px] font-bold uppercase">{{ $article->categories->first()->name }}</p>
                                    @endif
                                </div>
                                <img src="{{ Storage::url($article->thumbnail) }}" alt="thumbnail photo" class="object-cover w-full h-full" />
                            </div>
...
@empty
    <div class="col-span-1 md:col-span-3 flex flex-col items-center justify-center py-20">
        
        {{-- Icon Kaca Pembesar dengan Animasi Tailwind --}}
        <div class="bg-gray-100 p-6 rounded-full animate-bounce">
        ...
        <p class="text-xl font-semibold text-gray-500 mt-6 text-center">
            Ups! Tidak ditemukan "{{ ucfirst($keyword) }}"
        </p>
        <p class="text-gray-400 mt-2">Silakan coba kata kunci lain</p>
    </div>
@endforelse
